...
<p class="subNav">当前位置：首页&nbsp;>&nbsp;司机管理&nbsp;>&nbsp;司机详情<a href="javascript:void(0)" class="red1 ml30" onclick="javascript:history.go(-1);">返回</a></p>
<div class="mt20 ml30 black1 mr28 mb20">
	<p class="fw f14 f-yahei">司机详情</p>
	<p class="mt20 libg"><span class="fl ml20 lh28">基本信息</span></p>
	<input type="hidden" id="driverId" value="<s:property value='driverInfoEntity.driverId'/>"/>
	<ul class="mt20 black4 clearfix">
		<li class="fl mr25 listBorderRight">
		    <s:if test="%{driverInfoEntity.accountStatus==0}">
		    <p class="driverName p-r"><!-- 禁用的司机追加样式driverDisable --><s:property value="driverInfoEntity.driverName"/><em class="driverNameNo p-a"></em></p>
		    </s:if>
			<s:if test="%{driverInfoEntity.accountStatus==1}">
			<p class="driverName p-r driverDisable"><!-- 禁用的司机追加样式driverDisable --><s:property value="driverInfoEntity.driverName"/><em class="driverNameNo p-a"></em></p>
			</s:if>
			<p class="mt10 black4 f13 fw f-arial"><span class="display-ib driverTel"></span><s:property value="driverInfoEntity.telephone"/></p>
		</li>
		<li class="fl w176 line24 mr25">
			<p>司机性别：<s:property value="%{driverInfoEntity.sex==0?'男':'女'}"/></p>
			<p>
			账号状态：
			<s:if test="%{driverInfoEntity.accountStatus==0}">启用</s:if>
			<s:if test="%{driverInfoEntity.accountStatus==1}">禁用</s:if>
			</p>
			<p>出生日期：<em  <s:if test="%{null==driverInfoEntity.birthday||''==driverInfoEntity.birthday}">class="gray3"</s:if>><s:property value='%{(null==driverInfoEntity.birthday||""==driverInfoEntity.birthday)?"暂无信息":driverInfoEntity.birthday}'/></em></p>
		</li>
		<li class="fl w176 line24 mr25">
			<p>
			驾照类型：
			...
			<p>驾驶证号：<em  <s:if test="%{null==driverInfoEntity.driverNo||''==driverInfoEntity.driverNo}">class="gray3"</s:if>><s:property value='%{(null==driverInfoEntity.driverNo||""==driverInfoEntity.driverNo)?"暂无信息":driverInfoEntity.driverNo}'/></em></p>
			<p>年检日期：<em  <s:if test="%{null==driverInfoEntity.inspectionDate||''==driverInfoEntity.inspectionDate}">class="gray3"</s:if>><s:property value='%{(null==driverInfoEntity.inspectionDate||""==driverInfoEntity.inspectionDate)?"暂无信息":driverInfoEntity.inspectionDate}'/></em></p>
			
		</li>
		<li class="fl w176 line24">
			<p>身份证号：<em  <s:if test="%{null==driverInfoEntity.idNumber||''==driverInfoEntity.idNumber}">class="gray3"</s:if>><s:property value='%{(null==driverInfoEntity.idNumber||""==driverInfoEntity.idNumber)?"暂无信息":driverInfoEntity.idNumber}'/></em></p>
		</li>
	</ul>
	...
	<div class="mt10 clearfix t-c line35 p-r">
		...
		<em class="f-arial mr4 fw" id="beginSelectDate_str"></em>至<em class="f-arial ml4 mr4 fw" id="endSelectDate_str"></em>排班情况
...
{
	$.ajax({
		url : "../dispatchDriver/getDriverScheduleList.action",
		type : "post",
		dataType : "json",
		data : {
...
function doExport(){
	...
	window.location.href="../dispatchDriver/exportDriverScheduleList.action?driverId="+driverId+"&beginOrderDate="+beginOrderDate+"&endOrderDate="+endOrderDate+"&pageSize="+pageSize+"&currPage="+currPage+"&totalCount="+totalCount;
}
	...
	for(var i = 0;i < jsonDate.resultDate.length;i++){
		...
		for(var j = 0;j < jsonDate.resultDate[i].scheduleList.length;j++){
			var $div = "<div class='fl driverWorkList-text' id='line"+i+"div"+j+"'>"+
				...
				"<p class='ml25 mt10'>预计行驶<em class='red1 ml5 mr5 f-arial'>"+jsonDate.resultDate[i].scheduleList[j].lineTime+"</em>（"+jsonDate.resultDate[i].scheduleList[j].vehicleNumber+"）</p>"+
