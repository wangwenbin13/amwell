...
public class SystemLogListAction extends BaseAction {
	...
	private Logger logger = Logger.getLogger(SystemLogListAction.class);
	 ...
	 * TO_操作日志页面
	 */
	@Action(value = "toLogList", results = {
			@Result(name = "success", location = "/WEB-INF/view/syssetting/logList.jsp") })
	public String toLogList() throws Exception {
		currentPage = request.getParameter("currentPage") == null ? 0
	...
	public void exportExcel() {
		...
		String[] hearders = new String[] { "登录名", "用户姓名", "用户IP地址", "用户动作", "操作结果", "操作时间" };// 表头数组
		String[] fields = new String[] { "loginName", "userName", "userIp", "action", "operateResult", "operateTime" };// 对象属性数组
