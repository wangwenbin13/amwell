...
<div id="showPage1" class="showPage hide">
	<div class="pop black1 w960" id="popMapPage">
	    ...
	    <div class="pop-main p10">
	        <div>
	        	<input type="text" class="r-input w35p gray3" value="请输入位置点搜索" id="searchText" onfocus="if(value==defaultValue){value='';$(this).removeClass('gray3').addClass('gray2');}" onblur="if(!value){value=defaultValue;$(this).removeClass('gray2').addClass('gray3');}"/>
	        	<input type="submit" class="btn-blue4" value="查找" onclick="localSearch('nodetail')"/>
...
<div class="r-sub-nav-out"><p class="r-sub-nav gray2">当前位置：首页&nbsp;>&nbsp;线路列表<a class="display-ib btn ml10" href="javascript:void(0)" onclick="goBack();">返回</a><span class="blue1 ml25"></span></p></div>
  <div class="mt10 ml20 mr20 black1">
          <div class="fl widthfull p-r">
            ...
            <div class="tab_box">
            	<iframe frameborder="0" id="loadPage" name="loadPage" src="recruitLinesDetail.action" width="100%"></iframe>
            </div>
...
function changePage(index)
{
	if(index == 0)
	{
		...
		$("#loadPage").attr("src","recruitLinesDetail.action");
	}
	else if(index == 1)
	{
	    $("#loadPage").attr("src","recruitLinesOrderNum.action");
	}
}
//清除value值
$(function(){
...
function getCurrMarkerInfo(title,address,lat,lng)
{
	...
	if(inputId.indexOf("Start")!=-1){
	   ...
	   $("#lati"+inputId.substring(inputId.indexOf("Start"))).val(lat).removeClass("gray3").addClass("gray2");
	   $("#loni"+inputId.substring(inputId.indexOf("Start"))).val(lng).removeClass("gray3").addClass("gray2");
	}
	if(inputId.indexOf("End")!=-1){
	   
	   $("#lati"+inputId.substring(inputId.indexOf("End"))).val(lat).removeClass("gray3").addClass("gray2");
	   $("#loni"+inputId.substring(inputId.indexOf("End"))).val(lng).removeClass("gray3").addClass("gray2");
	}
	if(inputId.indexOf("Access")!=-1){
	   
	   $("#lati"+inputId.substring(inputId.indexOf("Access"))).val(lat).removeClass("gray3").addClass("gray2");
	   $("#loni"+inputId.substring(inputId.indexOf("Access"))).val(lng).removeClass("gray3").addClass("gray2");
	}
	closePopMapPage();
}
