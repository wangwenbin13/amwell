...
public class SysLogDaoImpl extends DaoSupport implements ISysLogDao {
	...
	public Map<String, Object> getSysLogList(Search search,int currentPage, int pageSize){
		...
		StringBuffer sql=new StringBuffer("select log.operateTime,log.operateResult,log.userIp,log.action,admin.userName,admin.loginName from sys_log log left join sys_admin admin on log.userId=admin.userId where 1=1  ");
		...
		sql.append("  and log.sysType like 0  order by log.operateTime desc  ");
