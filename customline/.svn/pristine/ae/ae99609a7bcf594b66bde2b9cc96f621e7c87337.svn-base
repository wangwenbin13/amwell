...
public class MyDate {
	public static void main(String args[]) {
		System.out.println(compare_time_app("2014-12-10 15:18:52", "2014-12-10 15:18:02"));
	}

	public static int compare_time_app(String time1, String time2) {
	 ...
	 * @param time
	 *            yyyy-MM
	 * @return map.get("frist_S") map.get("last_S") map.get("frist_L")
	 *         map.get("last_L")
	 */
	 ...
	 * @return
	 */
	public static long strToUTCLong(String strTime, String type) {// 2011-03-10
																	// 12:18:00
	...
	public static String getFirstDayOfMonth(String date) throws ParseException {
		...
		if (String.valueOf(forTime.getMonth() + 1).length() == 1)
			timeString += "0" + String.valueOf(forTime.getMonth() + 1);
		else
	 ...
	 * 获的两个时段相差的小时数
	 */
	public static int diffHours(long beginTime, long endTime) {
		long mss = (endTime - beginTime);
		long day = mss / (1000 * 60 * 60 * 24);
		long hours = (mss % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60);
		int diffHours = new Long(day).intValue() * 24 + new Long(hours).intValue();
		return diffHours;
	}
	...
	public static List<String> getBetweenDate(String d1, String d2) {
		...
		try {
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Calendar gc1 = Calendar.getInstance(), gc2 = Calendar.getInstance();
			...
			for (Calendar e : list) {
				int the_week = Calendar.DAY_OF_WEEK;
				if (e.get(the_week) == 1 || e.get(the_week) == 7) {
					continue;
				} else {
					resultList.add(sdf.format(e.getTime()));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
