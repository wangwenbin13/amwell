...
<title>营销管理-APP站内信-站内信模板</title>
<jsp:include page="../resource.jsp"/>
</head>

<body> 
<div class="table-title mt20"><a class="btn fr mr8 mt4"  id=showAddInsidePage href="javascript:void(0)">增加</a></div>
...
//增加短信模板
$("#showAddInsidePage").click(function(){	
		$("#iframe_right",parent.parent.window.document).attr("src","../insideMessage/insideAPPModelAdd.action?feedbackId");
});

//修改短信模板
function showEditMarketPage(id){
	$("#iframe_right",parent.parent.window.document).attr("src","../insideMessage/marketingModelEdit.action?msgTemplateVo.id="+id);
}
...
function deleteMarketPage(id){
	...
	$("#commonSure",parent.parent.window.document).click(function(){
		...
		$.ajax({
			url:'../insideMessage/marketingModelDel.action?msgTemplateVo.id='+id,
			type:'post',
			cache:false,	
			dataType:"text",	
