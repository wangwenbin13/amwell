...
<title>首页-运营报表-报表</title>
<jsp:include page="../resource.jsp"/>
</head>

<body>
  <div class="r-sub-nav-out"><p class="r-sub-nav gray2">当前位置：首页&nbsp;>&nbsp;运营报表&nbsp;>&nbsp;线路运营报</p></div>
  <div class="mt10 ml10 black1">
  
   <form name="" action="../financial/queryReportList.action" method="post" id="searchForm">
        ...
        <li class="w11p">
        	<span class="fl">得分：</span>
        	<s:textfield name="search.field06" theme="simple" cssClass="fl r-input gray2 w63p mt2"/>
		</li>
         
        <li class="w26p">
            <c:if test="${cityName==null||cityName==''}">
            <span class="fl">城市：</span><s:textfield name="search.field07" theme="simple" cssClass="fl r-input gray2 w32p mt2"/>
            </c:if>
        	<c:if test="${cityName!=null&&cityName!=''}">
        	<span class="fl">城市：</span>${cityName}<s:hidden name="search.field07" theme="simple" cssClass="fl r-input gray2 w32p mt2"></s:hidden>
        	</c:if>
       	...
       	<div style="overflow-x:auto;overflow-y:hidden">
            <table width="100%" border="0" class="table3" id="tableDiv">
              <tr align="center" class="th">
              	<th scope="col" width="2%">序号</th>
                <th scope="col" width="4%">日期</th>
                <th scope="col" width="5%">商家</th>
                <th scope="col" width="3%">城市</th>
                <th scope="col" width="5%">线路名称</th>
                <th scope="col" width="5%">司机姓名</th>
                <th scope="col" width="5%">司机电话</th>
                <th scope="col" width="5%">车牌</th>
                <th scope="col" width="5%">发车时间</th>
                <th scope="col" width="9%">起点</th>
                <th scope="col" width="9%">终点</th>
                <th scope="col" width="3%">座位数</th>
                <th scope="col" width="3%">出票数</th>
                <th scope="col" width="3%">退票数</th>
                <th scope="col" width="3%">改签数</th>
                <th scope="col" width="4%">实际上车数</th>
                <th scope="col" width="4%">上座率</th>
                <th scope="col" width="5%">评价得分</th>
                <th scope="col" width="6%">投诉原因前一</th>
                <th scope="col" width="6%">投诉原因前二</th>
                <th scope="col" width="6%">投诉原因前三</th>
              </tr>
              <s:if test="null==list || list.size == 0">
	              <tr align="center" class="bg1">
	   					<td colspan="20"><p class="f13 f-yahei gray4 t-c line26 mt10">
             ...
             <s:iterator value="list" var="financial" status="s">
						...
						<s:if test="%{#s.count%2 == 0}">
                    		...
                    		<td class="f-arial">${financial.driverName}</td>
                    		<td class="f-arial">${financial.telephone}</td>
                    		<td class="f-arial">${financial.vehicleNumber}</td>
                    		<td class="f-arial">${financial.orderStartTime}</td>
                    		<td class="f-arial">${financial.bstation }</td>
                    		<td class="f-arial">${financial.estation }</td>
                    		<td class="f-arial">${financial.orderSeats }</td>
                    		<td class="f-arial">${financial.buyNum }</td>
                    		<td class="f-arial">${financial.returnNum }</td>
                    		<td class="f-arial">${financial.changeNum }</td>
                    		<td class="f-arial">${financial.byBusNum }</td>
                    		<td class="f-arial">${financial.attendance }</td>
                    		<td class="f-arial">${financial.score }</td>
                    		<td class="f-arial">${financial.advOne }</td>
                    		<td class="f-arial">${financial.advTwo }</td>
                    		<td class="f-arial">${financial.advThree }</td>
                    	</tr>
                   	</s:iterator>
            </table>                  
          	</div>
...
<form action="../financial/queryReportList.action" method="post" style="display: none;" id="turnPage">
...
<form action="../financial/exportReportExcel.action" method="post" style="display: none;" id="export">
   	<input type="text" name="search.field01" value="${search.field01 }"/>
    <input type="text" name="search.field02" value="${search.field02}"/>
    <input type="text" name="search.field03" value="${search.field03}"/>
    <input type="text" name="search.field04" value="${search.field04}"/>
