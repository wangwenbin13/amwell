...
<title>系统设置-编辑角色</title>
<%@include file="../resource.jsp" %>
</head>
<body style="background:#fff;">
<form action="" method="post" id="roleForm">
<s:hidden theme="simple" name="sysRoleVo.roleId"></s:hidden>
<s:hidden theme="simple" value="%{sysRoleVo.roleName}" id="originalRoleNameUpdate"></s:hidden>
<s:hidden theme="simple" name="permissionCodes" id="permissionCodes"></s:hidden>
	<div class="pop black1" id="popEditPage" style="width:800px">
		<div class="pop-t">
	    	<div class="pop-t-l fl fw white">编辑角色</div>
	        <div class="pop-t-r fr"><a href="javascript:void(0);" onclick="closePopEditPage();" class="pop-close fr mt4 mr4"></a></div>
	    </div>
	    <div class="pop-main p10">
	        ...
	        <p class="t-c mt10 mb20"><input type="button" class="display-ib btn1 white mr40 f14" value="编&nbsp;辑" id="submitButton" onclick="validateFunction('roleForm');"/>
	        <a onclick="closePopEditPage();" href="javascript:void(0);" class="display-ib btn1 white f14">取&nbsp;消</a></p>
	    </div>
	</div>
</form>
</body>
...
function onCheck(e, treeId, treeNode) {
	...
	//
	if("" == permissIds){
		createErrorTip("treeDemo_2","权限不能为空");
		setSubmitDisales(true);
	}else{
		validateUserDefinedSucc("treeDemo_2");	
		//将提交按钮恢复
		setSubmitDisales(false);
	}
}
$(document).ready(function(){
	$.fn.zTree.init($("#treeDemo_2"), settings,  ${ztreeJson});
});
...
function validateFunction(submitForm){
	...
	//验证角色名称是否重复
	//第一个参数是验证的Id,第二个是值
	roleNameIsRepetition("roleName",$("#roleName").val());
	//去除左右两端的空格
	textTrim("roleName");
	//验证未通过的计数
...
function roleNameIsRepetition(verificationId,verificationValue){
	...
	var originalRoleName = $("#originalRoleNameUpdate").val();
	
	if(originalRoleName == verificationValue.trim()){
...
//获取树选中id
function checkNode() {
	var zTree = $.fn.zTree.getZTreeObj("treeDemo_2"),
...
function roleFormSubmit(submitForm){
	...
	$("#"+submitForm).ajaxSubmit({
		...
		success : function(data) {
			...
			if("toLogin" == data){
				window.top.location.href='../login/toLogin.action';
			}
		}
	});
}
