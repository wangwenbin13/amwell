...
<html>
  ...
  <body>
    ...
    <p class="t-c mb20 mt20">
	        <input type="button" value="保&nbsp;&nbsp;存" class="display-ib btn1 white f14 mr20" onclick="loadPage.window.saveSetDate()"/>
	        <input type="button" value="取&nbsp;&nbsp;消" class="display-ib btn1 white f14" onclick="loadPage.window.closePopDatePage();"/>
        </p>

        <s:form id="addForm" action="line/updateLineDate.action" method="post" theme="simple">
            <input id="lineBaseId" type="hidden" name="lineBaseId"/>
            <input id="orderStartTime" type="hidden" name="orderStartTime"/>
            <input id="orderSeat" type="hidden" name="orderSeat"/>
  			<input id="theNewClassTimeFlag" type="hidden" name="theNewClassTimeFlag" class="r-input t-c w75 mr5" value="0"/>
  			<!-- 排班日期 -->
  			<input id="orderDateStart" type="hidden" name="orderDate" class="r-input t-c w75 mr5"/>
  			<!-- 年月-->
  			<input id="yearAndMonth" type="hidden" name="yearAndMonth" class="r-input t-c w75 mr5"/>
  			<!-- 班次是否包月 -->
...
function saveSetDate()
{
	...
	if(allSelectDate==""){
		...
		parent.parent.showRturnTip("请选择工作时间！",false);
	}
	...
	$("#orderDateStart",parent.window.document).val(allSelectDate);
	$("#yearAndMonth",parent.window.document).val($("#txtSelectDate",parent.window.document).val());
	$("#theSupportMonth",parent.window.document).val(supportMonthValue);

	var $theAction=$("#addForm",parent.window.document).attr("action");
	//判断是否是IE浏览器
	if(navigator.userAgent.indexOf("MSIE")>0){   
		$theAction="../"+$theAction;
	}
	$("#addForm",parent.window.document).attr("action",$theAction);
	
    $("#addForm",parent.window.document).ajaxSubmit({
		type : 'post',
...
$(function(){	
    ...
    $("#lineBaseId",parent.window.document).val(lineBaseId);
    orderStartTime="<%=request.getParameter("orderStartTime")%>";
    $("#orderStartTime",parent.window.document).val(orderStartTime);
    orderSeat="<%=request.getParameter("orderSeat")%>";
    $("#orderSeat",parent.window.document).val(orderSeat);
