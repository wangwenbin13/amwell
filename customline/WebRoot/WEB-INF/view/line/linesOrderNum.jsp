...
<div id="showPagePic" class="showPage"></div>
    <div class="r-sub-nav-out"><p class="r-sub-nav gray2">当前位置：首页 &gt; 发布线路 <span class="blue1 ml25"></span><span><a class="gray2" href="javascript:history.go(-1);">返回</a></span></p></div>
    <div style="padding:10px;">
    <!-- 隐藏线路信息 -->
    <input id="lineBaseId" type="hidden" value="${lineScheduleInfo.lineBaseId}">
    <p class="pop-main-tips mt10 gray2">线路信息：${lineScheduleInfo.lineName}</p>
    <p class="mt10 gray2 line24"><span class="fl w116 t-r">选择数据导出时间：</span><span class="fl r-input w170 t-l"><input type="text" id="startTime"  onclick="WdatePicker({position:{left:-9,top:4},dateFmt:'yyyy-MM-dd'})" readonly="readonly" class="Wdate75 gray2 Wdate"></span><span class="fl ml5 mr5">至</span><span class="fl r-input w170 t-l"><input type="text" id="endTime"  onclick="WdatePicker({position:{left:-9,top:4},dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startTime\');}'})" readonly="readonly" class="Wdate75 gray2 Wdate"></span>&nbsp;&nbsp;<input type="checkbox" id="isRomoveRepeat" class="checkbox ml5" value="1">是否去重&nbsp;&nbsp;<button class="btn-blue4 ml10" onclick="exportLinePassengers();">导出</button></p>
    	...
    	<s:iterator value="lineScheduleInfo.lineClassList" var="c" status="s">
    	  <input type="radio" name="className" class="checkbox mr5" value="${c.orderStartTime}" <s:if test="#s.index==0">checked="checked"</s:if> onclick="getDiffDateInfo(${s.index},'${c.orderStartTime}')"/>${c.orderStartTime}&nbsp;&nbsp;${c.orderSeats}位
   		...
   		<div style="left: 50px; top: 29px;" class="dj-box p-r p-a">
	        <span class="arrow2 p-a" style="left: 7px;" id="arrowIcon" ></span>
	        <p class="mt15">
...
function exportLinePassengers(){
	orderStartTime = $("input[name='className']:checked").val();
	var startTime = $("#startTime").val();
	var endTime = $("#endTime").val();
	var isRomoveRepeat = $("#isRomoveRepeat").attr("checked")==undefined?0:1;
	if(startTime==''||endTime==''){
	   parent.parent.showRturnTip("导出数据的时间段必须选择",false); 
	   return;
	}
	window.location.href="line/exportLinePassengers.action?lineBaseId="+lineBaseId+"&orderStartTime="+orderStartTime+"&startTime="+startTime+"&endTime="+endTime+"&isRomoveRepeat="+isRomoveRepeat;
	
}
//订购人数详情页面
function goOrderNumDetail(orderDay){
	$("#topHide",parent.window.document).show();
    $("#leftHide",parent.window.document).show();
    $("#showPage").load("line/getOrderNumDetail.action?random="+Math.floor(Math.random()*10000+1),{lineBaseId:lineBaseId,orderStartTime:orderStartTime,orderDate:orderDay}); 
    $("#mainBody").show();
}
...
function loadData(lineBaseId,orderStartTime,orderDate){
    var url = "line/getClassMonthOrderCount.action?lineBaseId="+lineBaseId+"&orderStartTime="+orderStartTime+"&orderDate="+orderDate;
  	$.ajax({
		url:url,		
		cache:false,	
		dataType:"json",
	...
	for(var j = 1;j<8;j++){
		...
		if(weekDay == $("#date_time"+j).attr("week"))
		{
			...
			for(var m = 0;m < monthDays;m++)
			{
				...
				$("#date_num"+parseInt(m+j)).html(m+1+"<p class='mt7' id='defaultText'><span class='gray3'>未订购</span></p>");
				for(var n = 0; n < jsonDate.length;n++)
				{
					if(jsonDate[n].dateTime == (showYear+"-"+showMonth+"-"+parseDate(m+1)))
					{
						...
						$("#date_num"+parseInt(m+j)).append("<p class='mt7'><a href='javascript:void(0);' class='blue3 cursor' onclick='goOrderNumDetail(\""+(selectYear+"-"+parseDate((m+1)))+"\")'>"+jsonDate[n].lineNum+"</a></p>");
					}
				}
			}
		}
