...
function saveSetDate()
{
	...
	//保存年月信息
	$("#"+theYearAndMonth).val($("#txtStartDate",parent.window.document).val());
	closePopDatePage();	
...
$(function(){	
    ...
    theYearAndMonth="<%=request.getParameter("theYearAndMonth")%>";
...
function showDate()
{
	...
	$("#currYear_0",parent.window.document).text(currYear);
	...
	for(var j = 1;j<8;j++){
		...
		if(weekDay == $("#"+id+"date_time"+j,parent.window.document).attr("week"))
		{
			...
			for(var m = 0;m < monthDays;m++){
				//遍历一个月的天
				var checkItem1 = $("#"+id+"date_time"+parseInt(m+j),parent.window.document);
				if(checkItem1.attr("week") == "星期六" || checkItem1.attr("week") == "星期天"){
					checkItem1.html((m+1)+"<p class='t-c'><input type='checkbox' class='checkbox' name='dateCheck' value='"+(showYear+"-"+showMonth+"-"+parseDate((m+1)))+"'/></p>");
				}
				else{
					checkItem1.html((m+1)+"<p class='t-c'><input type='checkbox' class='checkbox' name='dateCheck' value='"+(showYear+"-"+showMonth+"-"+parseDate((m+1)))+"' checked='checked'/></p>");
				}
				if(selectYear == currYear){
					if( m == dayIndex){
						// 当天 则改变背景颜色
						var checkItem = $("#"+id+"date_time"+parseInt(m+preCount),parent.window.document);
						checkItem.css({"background":"#e9ad1d"});
					}else if(m < dayIndex){
						// 把格子置灰并且不可选择
						var checkItem = $("#"+id+"date_time"+parseInt(m+preCount),parent.window.document);
						checkItem.css({"background":"#e7e5e5"});
						checkItem.children("p").find("input").attr("checked",false);
						checkItem.children("p").find("input").attr("disabled","disabled");
					}
				}
			}
		}
