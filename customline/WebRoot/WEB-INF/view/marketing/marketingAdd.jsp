...
<body>
<s:form id="form2" action="../marketing/addMarketingMsg.action" method="post" theme="simple">
<div class="table2-text table2-text-height">
	<div class="xinxi-box">
	    <table width="100%" border="0" class="line24">
	        <tr>
	            <th scope="row" class="f14 fw xinxi-box-t">发送短信</th>
	            <td valign="top">
	                <ul class="mt15 ml10">
	                    ...
	                    <li class="mb10 clearfix"><span class="w75 t-r fl">内容：<em class="red1 t-r mr4">* </em></span>
	                    	<textarea class="r-input more-show fl"  id="content" style="width:500px;height:100px;" name="search.field02" check="11"></textarea>
		                	<input type="hidden" name="content" value=""/>
	                    </li>
	                   
	                	<li class="w58p mb10 clearfix">
	                     	 <span class="fl w75 t-r">发送时间：</span>
	                     	 <span class="fl"><span onclick="" class="black1 ml10"><input type="radio" class="checkbox mr4" name="search.field05" checked="checked" value="0" id="lijifasong"/></span>立即发送</span>
	                  	...
	                  	<li class="w58p clearfix">
	                        <span class="fl w75 t-r mr10">服务商：</span>
	                     	<s:radio cssClass="checkbox mr4"  list="#{'YIMEI':'亿美','GUODU':'国都','CHUANGLAN':'创蓝'}" value="亿美" name="provider" id="provider" onclick="checkBusniness();" ></s:radio>
	                     		<span class="ml40 gray1" >（剩余短信:<em class="fw red1"  id="targetCount">${msgCount }</em>）</span><s:if test="%{msgCount<500 && msgCount>0}"><span class="ml40 red1" >(剩余的短信已不多，请及时充值)</span></s:if>
				                <s:if test="%{msgCount<0 }"><span class="ml40 red1" >（查询剩余短信条数失败）</span></s:if>
	                     </li>
	                	<li class="w99p mb10 clearfix">
	                        <div class="sb-top p-r" id="sb-top">
				            	<span class="w75 t-r">发送人群：</span>
				            	<span onclick="changePage(0);" class="black1"><input type="radio"   class="checkbox" name="search.field04" value="3"/></span><em class="ml4">全部用户</em>		
				                <span class="black1 ml10" onclick="changePage(1);"><input type="radio"   class="checkbox" name="search.field04" value="1"/></span><em class="ml4">选择发送</em><a href="javascript:void(0);" onclick="" class="blue1 ml10"></a>
	                ...
	                <div class="ml20 mr20 mb10 hide p-r" id="leadDiv">
	                	...
	                	<div class="dj-box p-r" style="margin-left: 72px;width:540px;background:#fff" id="leadDiv1">
	...
	<input type="button" onclick="cancle()" class="display-ib btn1 white" value="取消"/></p>
</div>
</s:form>
</body>
</html>
...
$(function() {
	$('#providerYIMEI').attr('checked', 'checked');
	checkBusniness()
	});
function cancle(){
	$("#iframe_right",parent.parent.window.document).attr("src","../marketing/marketingManager.action?level=two&toAdd=0");
}
//创建添加问题的文字编辑器
var editor;
...
function queryTemplateContent(){
	...
	if(typeof(v)=="undefined" && v == "" ){
	 ...
	 $.ajax({
			url:'../marketing/queryMarketingModel.action?random='+Math.floor(Math.random()*10000+1),
			cache:false,
			data:{"msgTemplateVo.id":v},
			type : 'post',
			dataType:"json",
			success:function(data){	
// 				alert(data[0].template_content);
				if(data != null && data != ''){
...
function sendMessage(){
	...
	$("#form2").ajaxSubmit({
	type:'post',
	data:{"phoneNumber":phoneNumber, "provider": provider },
	success : function(data){
		data = eval("(" + data + ")");
...
function uploadFile(){
   ...
   $.ajaxFileUpload({
			...
			success : function(data) {
				data = eval("(" + data + ")");
// 				alert(data);
				var theSameTelephone=data.samePhones;
				var wrongTelephone = data.wrongTelephone;
				var count=data.count;
// 				alert(theSameTelephone+"==="+wrongTelephone);
				if (data.result == "error") {
					parent.parent.showRturnTip("导入失败",false);
				} else {
					parent.parent.showRturnTip("成功导入"+count+"条数据",true);
...
function checkBusniness(){
	var provider = $("[name='provider']:checked").val();
	$.ajax({
		url:'../marketing/checkMsgBusiness.action?random='+Math.floor(Math.random()*10000+1),
		cache:false,
