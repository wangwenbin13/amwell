...
<title>营销管理-APP消息-APP消息列表</title>
<jsp:include page="../resource.jsp"/>
</head>

<body>
   <s:form name="" action="../pushMessage/pushAPPMsgList.action" method="post" theme="simple">
   	 <ul class="r-sub-sel black1 mt20 clearfix">
        <li class="w30p">
           	<span class="fl">时间：</span>
            <span class="r-input fl w33p mr10"><input type="text" name="search.field02" value="${search.field02 }" class="Wdate75 gray2 Wdate" readonly="readonly" id="txtStartDate" onclick="WdatePicker({position:{left:-9,top:4},dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'txtEndDate\');}'})"/></span>
            <span class="fl">至</span>
            <span class="r-input fl w33p ml10"><input type="text" name="search.field03" value="${search.field03 }" class="Wdate75 gray2 Wdate" readonly="readonly" id="txtEndDate" onclick="WdatePicker({position:{left:-9,top:4},dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'txtStartDate\');}'})"/></span>
        </li>
        <li class="w39p"><span class="fl">内容关键字：</span><input type="text" name="search.field01" value="${search.field01}" class="r-input w58p gray2"/>
              ...
              <table width="100%" border="0" class="table1" id="tableDiv">
				...
				<s:iterator value="list" var="marketingVo" status="s">
					<tr  align="center" marketingId=${marketingVo.msgId } class="cursor">
						...
						<td class="f-arial"><span id="context${s.index }" title="${marketingVo.msgContext}"><script type="text/javascript">$("#context"+${s.index }).text(Util.cutstr('${marketingVo.msgContext }',30))</script></span> </td>
					</tr>
				</s:iterator>
                  </table>
    ...
    <form action="../pushMessage/pushAPPMsgList.action" method="post" style="display: none;" id="turnPage">
...
$("#tableDiv tr td").click(function(){
	...
	{
		var messageId = $(this).parent().attr("marketingId");
		//alert(feedbackId);
		//跳转到帮助反馈详情

		$("#iframe_right",parent.parent.window.document).attr("src","../pushMessage/pushAPPMsgDetail.action?messageId="+messageId);
	}
});
