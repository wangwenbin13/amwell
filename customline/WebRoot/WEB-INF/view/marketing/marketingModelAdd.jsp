...
<div class="r-sub-nav-out"><p class="r-sub-nav gray2">当前位置：首页&nbsp;>&nbsp;短信管理&nbsp;>&nbsp;增加短信模板</p></div>
<div class="mt10 ml20 mr20 black1">
 	<div class="table-title">
        <ul>
        	<li class="on"><a href="javascript:void(0)" class="f14 blue2">增加模板</a></li>
        </ul>
    </div>
    <ul class="mt25 ml10">
      	<li class="w58p mb10 clearfix"><span class="w75 t-r fl">模板名称：</span>                		
          	<s:textfield type="text" theme="simple" cssClass="r-input w50p fl" name="msgTemplateVo.template_name" id="template_name" onkeydown="textCounts()" onkeyup="textCounts()" check="11"></s:textfield>
      	</li>
          <li class="w58p mb10 clearfix"><span class="w75 t-r fl">模板内容：</span>
          	<s:textarea theme="simple" cssClass="r-input more-show w85p fl"  id="template_content"  style="height:100px;" name="msgTemplateVo.template_content" check="11"></s:textarea>
          </li>
	</ul>	
	<p></p>
	<p class=" mt20"><span class="w75 t-r fl"></span><a class="display-ib btn1 white mr40 ml10" href="javascript:void(0)" id="submitButton">保存</a>
	<input type="button" class="display-ib btn1 white" value="取消" onclick="cancle()"/></p>               
 </div>
</s:form>
</body>
</html>
...
function cancle(){
	$("#iframe_right",parent.parent.window.document).attr("src","../marketing/marketingManager.action?level=two&toAdd=2");
}
function adminFormSubmit(){
	...
	$("#addSMSTemplate").ajaxSubmit({
		type : 'post',
		data:{},
		url : "../marketing/addTemplateModel.action?random="+Math.floor(Math.random()*10000+1),
		dataType:"text",
...
//
 function textCounts(){
 	var counts=$("#template_name").val();
 	if(counts.length>30){
 		counts=counts.substring(0,30);
 		$("#template_name").val(counts);
 	}else{
