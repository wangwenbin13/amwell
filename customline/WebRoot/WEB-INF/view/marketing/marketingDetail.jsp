...
<div class="r-sub-nav-out"><p class="r-sub-nav gray2">当前位置：首页&nbsp;>&nbsp;短信管理&nbsp;>&nbsp;短信详情</p></div>
<div class="mt10 ml20 mr20 black1">
 	<div class="table-title">
        <ul>
        	<li class="on"><a href="javascript:void(0)" class="f14 blue2">短信详情</a></li>
        </ul>
    </div>
    <div class="sh-add-new map-shebei-li gray2 f12" >
    	 <form action="" id="ajaxForm" method="post"></form>
    <s:iterator value="marketingBean" var="msg" status="s">
    	<ul class="marLi clearfix f12">
            <li><span class="fl w62 t-r">发送时间：</span>${msg.createOn }</li>
            <li><span class="fl w62 t-r">操作人：</span>${msg.createBy }</li>
            <li><span class="fl w62 t-r">发送数量：</span>${msg.msgCount }</li>
            <li class="widthfull" style="height:auto;"><span class="fl w62 t-r">短信内容：</span><div style="overflow:hidden;">${msg.msgContext }</div></li> 
        </ul>
         </s:iterator>
         <div class="f12">
	        <span class="fl w62 t-r">发送人群：</span>
	        <div class="marketTable" style="overflow:hidden;">
				<s:if test="%{#msg.msgTarget==0}">
					<table width="100%" border="0" class="table1" id="tableDiv">
						...
						<s:iterator value="marketingBean.passengerList" var="passenger" status="s">
							<tr align="center">
								...
								<td>${msg.orderDate }</td>
								<td>${msg.orderStartTime }</td>
								<td>${msg.lineName }</td>
							</tr>
						</s:iterator>
					</table>
				</s:if>
				<s:if test="%{#msg.msgTarget==2}">
					<table width="100%" border="0" class="table1" id="tableDiv">
						<tr align="center" class="th">
							<th scope="col" width="50%">商家名称</th>
							<th scope="col" width="50%">手机号码</th>
						</tr>
						<s:iterator value="marketingBean.businessList" var="passenger" status="s">
							<tr align="center">
								<td>${passenger.name }</td>
								<td>${passenger.contactsPhone }</td>
							</tr>
						</s:iterator>
					</table>
				</s:if>
