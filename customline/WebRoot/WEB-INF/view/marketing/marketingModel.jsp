...
<title>营销管理-短信管理-短信模板</title>
<jsp:include page="../resource.jsp"/>
</head>

<body> 
<div class="table-title mt20"><a class="btn fr mr8 mt4" id="showAddMarketPage" href="javascript:void(0)">增加</a></div>
...
//增加短信模板
$("#showAddMarketPage").click(function(){	
		$("#iframe_right",parent.parent.window.document).attr("src","../marketing/marketingModelAdd.action?feedbackId");
});


//修改短信模板
function showEditMarketPage(id){
	$("#iframe_right",parent.parent.window.document).attr("src","../marketing/marketingModelEdit.action?msgTemplateVo.id="+id);
}
...
function deleteMarketPage(id){
	...
	$("#commonSure",parent.parent.window.document).click(function(){
		...
		$.ajax({
			url:'../marketing/marketingModelDel.action?msgTemplateVo.id='+id,
			type:'post',
			cache:false,	
			dataType:"text",	
