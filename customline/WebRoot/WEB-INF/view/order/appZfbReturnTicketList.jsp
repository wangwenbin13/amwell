...
<body>
  <div class="mt10 black1">
   ...
   <form id="searchForm" action="../returnTicket/appZfbReturnTicketList.action" method="post">
        ...
        <li class="w16p">
        	<span class="fl">支付宝交易号：</span>
        	<input type="text" class="fl r-input gray2 w63p mt2" name="search.field04" value="${search.field04}"/>
		</li>
        <li class="w16p"><span class="fl">退款状态：</span>
        	<s:select cssClass="w60p p3" list="#{2:'所有方式',0:'未退款',1:'已退款'}" theme="simple" name="search.field05">
	        </s:select>
        </li> 
        <li class="w16p"><span class="fl">支付宝交易号为空：</span>
        	<s:select cssClass="w60p p3" list="#{2:'所有方式',0:'是',1:'否'}" theme="simple" name="search.field06">
	        </s:select>
        </li> 
        <li><input type="submit" class="btn-blue4 ml20" value="查找" />
        <a href="javascript:void(0);" class="blue1 ml10" onclick="toExport();">下载搜索结果</a>
        <a href="javascript:void(0);" class="blue1 ml10" onclick="toReturnMoneyFromZfb();">支付宝退款</a>
        </li>
       	...
       	<div style="overflow-x:auto;overflow-y:hidden">
            <table width="100%" border="0" class="table1">
              <tr align="center">
                ...
                <th scope="col" width="15%">支付宝交易号</th>
                <th scope="col" width="8%">实际退票金额</th>
                <th scope="col">退票日期</th>
                <th scope="col" width="10%">退款时间</th>
                <th scope="col" width="6%">退款状态</th>
              </tr>
			...
			<s:iterator value="list" var="applyReturn" status="s">
				...
				<td class="f-arial">${currentPageIndex+s.count}</td>
             		...
             		<td class="f-arial">${applyReturn.thirdparty}</td>
             		<td class="f-arial">${applyReturn.realBackMoney}</td>
             		<td class="f-arial">${applyReturn.returnDates}</td>
             		<td class="f-arial">${applyReturn.operTime}</td>
             		<td class="f-arial">${applyReturn.returnOrNot}</td>
             	</tr>
               	</s:iterator>
            </table>                  
          	</div>
...
<form action="../returnTicket/appZfbReturnTicketList.action" method="post" style="display: none;" id="turnPage">
 ...
 <form action="../returnTicket/exportAppZfbExcel.action" method="post" style="display: none;" id="export">
   	<input type="text" name="search.field01" value="${search.field01 }"/>
    <input type="text" name="search.field02" value="${search.field02}"/>
    <input type="text" name="search.field03" value="${search.field03}"/>
    <input type="text" name="search.field04" value="${search.field04}"/>
 ...
 <form action="../returnTicket/toReturnMoneyFromZfb.action" method="post" style="display: none;" id="toReturn">
   	<input type="text" name="search.field01" value="${search.field01 }"/>
    <input type="text" name="search.field02" value="${search.field02}"/>
    <input type="text" name="search.field03" value="${search.field03}"/>
    <input type="text" name="search.field04" value="${search.field04}"/>
...
//退款
function toReturnMoneyFromZfb(){
	var listSize = "${listSize}";
	if(0==listSize){
		parent.parent.showCommonTip("没有需要退款的数据!");
		return;
	}
	$("#toReturn").submit();
}
