   ...
   <form name="" action="" method="post" id="searchform">
   	 ...
   	 <ul class="r-sub-sel black1 mt20 clearfix">
       	<li class="w30p">
            ...
            <input type="text" class="Wdate75 gray2 Wdate" name="search.field01" value="${search.field01 }" readonly="readonly" id="txtStartDate" onclick="WdatePicker({position:{left:-9,top:4},dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'txtEndDate\');}'})"/>
            </span>
            <span class="fl">至</span>
            <span class="r-input fl w33p ml10">
            <input type="text" class="Wdate75 gray2 Wdate" name="search.field02" value="${search.field02 }" readonly="readonly" id="txtEndDate" onclick="WdatePicker({position:{left:-9,top:4},dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'txtStartDate\');}'})"/>
            </span>
        </li>
        <li>
       	...
       	<div style="overflow-x:auto;overflow-y:hidden">
              <table width="100%" border="0" class="table3" id="tableDiv">
                    <tr align="center" class="th">
                      ...
                      <th scope="col" width="5%">金额</th>
                      <th scope="col" width="5%">班次</th>
                      <th scope="col" width="10%">上车点</th>
                      <th scope="col" width="10%">下车点</th>
                      <th scope="col" width="5%">乘坐日期</th>
                    </tr>
                    ...
                    <s:iterator value="list" var="passengerTicket" status="s">
						...
						<s:if test="%{#s.count%2 == 0}">
							...
							<td class="f-arial">${passengerTicket.leaseOrderTime}</td>
							<td class="f-arial">${passengerTicket.leaseOrderNo}</td>
                    		<td class="f-arial">${passengerTicket.nickName }</td>
                    		<td class="f-arial">${passengerTicket.telephone}</td>
                    		<td class="f-arial">${passengerTicket.lineName}</td>
                    		<td class="f-arial">${passengerTicket.totalCount}元</td>
                    		<td class="f-arial">${passengerTicket.orderStartTime}<input type="hidden" id="stationNames" value="${passengerTicket.stationName}"/></td>
                    		<td class="f-arial"></td>
                    		<td class="f-arial"></td>
                    		<td class="f-arial">
                    		<input type="hidden" id="theOrderDate" value="${passengerTicket.orderDate}"/>
...
$(function(){
	...
	$("#tableDiv tr").each(function(){
		if($(this).attr('class')!='th'){
			...
			//获取上车点和下车点信息
			var $stationNames=$(this).find("#stationNames").val();
			if(undefined!=$stationNames&&''!=$stationNames){
              var $names=$stationNames.split('-');
              $(this).find('td:eq(8)').html($names[0]);
              $(this).find('td:eq(9)').html($names[$names.length-1]);
			}
		}
	});
});
...
function goDateDetailPage(trObj)
{
    ...
    $("#showPage",parent.window.document).load("../operationPassenger/showDateDetail.action?theListType=1&orderDates="+trObj+"&random="+Math.floor(Math.random()*10000+1)); 
