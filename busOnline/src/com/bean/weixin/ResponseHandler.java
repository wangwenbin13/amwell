...
import com.util.weixin.XMLUtil;
...
public class ResponseHandler {
	...
	public void doParse(String xmlContent) throws JDOMException, IOException {
		...
		while (it.hasNext()) {
			Entry entry = (Entry) it.next();
			this.setParameter((String) entry.getKey(), (String) entry
					.getValue());
		}
	}
	...
	public boolean isWXsign() {
		StringBuffer sb = new StringBuffer();
		Hashtable<String, String> signMap = new Hashtable<>();
		Set es = this.parameters.entrySet();
		Iterator it = es.iterator();
		while (it.hasNext()) {
			Map.Entry entry = (Map.Entry) it.next();
		...
		signMap.put("appkey", PropertiesConfig.WEIXIN_PAY_SIGN_KEY);
