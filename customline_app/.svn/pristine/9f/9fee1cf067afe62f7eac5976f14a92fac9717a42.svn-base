...
public class LoginAndRegister extends BaseAction {
	 ...
	 * @throws IOException
	 */
	@Action(value = "checkMobile", results = { @Result(type = "json") })
	public String checkMobile() throws IOException {
		String mobile = request.getParameter("mobile");
		String ns = request.getParameter("ns") == null ? "0" : request.getParameter("ns");
		String flag = loginAndRegisterService.checkMobile(mobile);

		AppVo_3 vo = new AppVo_3();
		if (flag.equals("0")) {// 未注册
			vo.setA1("1");
			vo.setA3("");
		} else {// 已注册
			vo.setA1("0");
	...
	public String weiXinLogin() throws IOException {
		...
		vo.setA15(appUser.getSessionId());
		write(vo);
		return null;
	}
